...
export default function CollectionDetail() {
  ...
  useEffect(() => {
    if (router.query.slug) {
      ...
      formData.append('handle', router.query.slug);
      setTag(router.query.slug)
  ...
  useEffect(() => {
    if (tag) {
      ...
      if (productType == 'all') {
        ...
        formData.append('query', 'status:active AND tag:' + tag + " AND product_type:" + productType)
      }
      fetch(productURL, {
        method: "post",
        body: formData
  ...
  return (
    <div className="collection-detail_page">
      ...
      {
        mainData
          ? <div className="hero-section" style={{ background: "url(" + mainData.image.src.replace('.jpg', '_2048x.jpg') + ")" }}>
