...
export default function Home({ data }) {
  ...
  return (
    <div className="homepage">
      ...
      <div className="hero"
        style={{
          backgroundImage: heroData?.mobile.image && windowWidth && windowWidth < 576
            ? (heroGradientStyle + 'url(' + heroData.mobile.image.url + ')')
            : heroData && (heroGradientStyle + 'url(' + heroData.image.url + ')')
        }}
      >
        {
          windowWidth && windowWidth < 576 && heroData.mobile.video
          && <video
            autoPlay="autoplay"
            loop="loop"
            muted
              ...
              <Link passHref={true} href={item.product_row_url}>
                <a>
                  <div className="category-item round">
                    ...
                    <div className="hover-title p-4">{item.product_row_img_title}</div>
                  </div>
                </a>
              </Link>
            </div>
